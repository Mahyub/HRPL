...
/*---------------------------------------------------------------------------*/
PROCESS(unicast_sender_process, "HRPL -- Point to Point Application");
         ...
         uint16_t datalen)
{
  ...
  if(data.ping) {
    printf("App: received ping from %u\n",data.src);
  } else {
    printf("App: received pong from %u\n",data.src);
  }
...
void app_send_to(uint16_t id, int ping, uint32_t seqno) {
  ...
  if(ping) {
    printf("App: sending ping to %u\n",data.dest);
  } else {
    printf("App: sending pong to %u\n",data.dest);
  }
  rpl_trace(&data);
...
PROCESS_THREAD(unicast_sender_process, ev, data)
{
  ...
  if(node_id == ROOT_ID) {
    ...
    etimer_set(&periodic_timer, TIME_BEFORE_APP *  CLOCK_SECOND);
