...
MEMB(heapNodes,struct MinHeapNode,N_NODES);
/*---------------------------------------------------------------------------*/
/* A utility function to create a new Min Heap Node */
static struct MinHeapNode* newMinHeapNode(int v, int dist)
{
	struct MinHeapNode* minHeapNode = memb_alloc(&heapNodes);
	if (minHeapNode==NULL){
		printf("Mahyoub could not allocate new minHeapNode\n");
	}
	minHeapNode->v = v;
...
void hrpl_rt_mangr_dijkstra(int src, int target)
{
	...
	free(minHeap->pos);
	for (v = 0; v < nodes_num; ++v){
		memb_free(&heapNodes,minHeap->array[v]);
	}
	free(minHeap->array);
	free(minHeap);
	free(dist);
